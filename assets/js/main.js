 ...
 * Transcipt
 * ======================================================================== */
let transcriptButtons = document.getElementsByClassName('transcript-text__button');
let Opened = false;

for (let i = 0; i < transcriptButtons.length; i++) {
  transcriptButtons[i].addEventListener('click', function() {
        this.classList.toggle('active');
        Opened = true;
        let transcriptContent = this.nextElementSibling;
        
        if (transcriptContent.style.maxHeight) {
          transcriptContent.style.maxHeight = null;
        }
         else {
          transcriptContent.style.maxHeight = transcriptContent.scrollHeight + "px";
        }
    });
}
