...
var SCORMWrapper = ( function () {
  ...
  function _updateDomStatus( value ) {
    document.querySelector( 'body' ).setAttribute( 'data-status', value );
  }
  ...
  function initializeInternal() {
    if ( typeof pipwerks !== 'undefined' ) {
      ...
      _SCORM = pipwerks.SCORM;
      _SCORM.version = '1.2';
      _isActive = _SCORM.init();

      if ( _isActive ) {
        status = _SCORM.get( 'cmi.core.lesson_status' );

        if ( status === undefined || status === '' || status === 'not attempted' ) {
          status = 'incomplete';
        }

        setInternal( 'cmi.core.lesson_status', status );
      }
    }
  }
  ...
  function setInternal( key, value ) {
    ...
    if ( _isActive && key && value ) {
      str = _SCORM.set( key, value );
      _SCORM.save();

      if ( key === 'cmi.core.lesson_status' ) {
        _updateDomStatus( value );
      }
    }
  ...
  return {
    ...
    terminate : terminateInternal,
    get : getInternal,
    set : setInternal,
    isActive01 : function () { return _isActive; }
  };

} )();

if ( typeof SCORMWrapper === 'undefined' ) {
...
window.addEventListener( 'load', function () {
  ...
  document.addEventListener( 'scormCompleted', function () {
    SCORMWrapper.set( 'cmi.core.lesson_status', 'completed' );
